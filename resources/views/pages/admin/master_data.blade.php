    ...
    <section class="pl-24 pr-20 my-10 ml-56 flex-grow">
        <div class="flex justify-between items-center mb-4">
            ...
            <button id="addDataButton"
                class="bg-blue-500 text-white px-4 py-2 rounded flex items-center hover:bg-blue-600 transition duration-150 ease-in-out"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    <a href="{{ url('master-data/' . $contact->id . '/edit') }}"
                                        class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 transition duration-150 ease-in-out">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="{{ url('master-data/' . $contact->id) }}"
                                        class="bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600 transition duration-150 ease-in-out">
                                    ...
                                    <form action="{{ url('master-data/' . $contact->id) }}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 transition duration-150 ease-in-out"
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
                ...
                <form id="dataForm" action="{{ url('master-data') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="division_id" name="division_id" class="w-full px-3 py-2 border rounded"
                            onchange="filterSubdivisions()">
                            @foreach ($divisions as $division)
                                <option value="{{ $division->id }}">{{ $division->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex justify-end">
                        ...
                        <button type="submit"
                            class="bg-blue-500 text-white px-4 py-2 rounded ml-2 hover:bg-blue-600 transition duration-150 ease-in-out">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
