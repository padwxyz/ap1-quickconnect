        ...
        <div class="bg-white p-6 rounded-lg shadow-lg">
            ...
            <form action="{{ url('master-data/' . $contact->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <select id="division_id" name="division_id" class="w-full px-3 py-2 border rounded"
                        ...
                        @foreach ($divisions as $division)
                            <option value="{{ $division->id }}"
                                {{ $division->id == $contact->division_id ? 'selected' : '' }}>{{ $division->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="wa_link" name="wa_link" value="{{ $contact->wa_link }}"
                        class="w-full px-3 py-2 border rounded">
                </div>
                <div class="flex justify-end">
                    <button type="button"
                        class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 transition duration-150 ease-in-out"
                        onclick="window.location.href='{{ url('master-data') }}'">Batal</button>
                    <button type="submit"
