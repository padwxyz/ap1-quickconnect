    ...
    <div class="bg-white p-6 rounded-lg shadow">
        ...
        <ul class="divide-y divide-gray-200">
            ...
            <li class="py-4 flex justify-between items-center">
                <div>
                    <h3 class="text-lg font-semibold text-gray-700">{{ $contact->name }} - {{ $contact->division->name }}</h3>
                    <p class="text-gray-500">{{ $contact->subdivision->name }}, {{ $contact->created_at->diffForHumans() }}</p>
                </div>
                <a href="{{ url('master-data/' . $contact->id . '/edit') }}" class="text-blue-500 hover:text-blue-600">Detail</a>
            </li>
            @endforeach
        </ul>
    </div>
